...
plotMatrix(
    refocus(hic, 'II'),
    use.scores = 'balanced', limits = c(-4, -1), dpi = 120, 
    max.distance = 200000
)
```
...
```{r}
aggr_centros <- aggregate(hic, targets = loops)
plotMatrix(aggr_centros, use.scores = 'detrended', limits = c(-1, 1), scale = 'linear', cmap = bgrColors())
```
...
p <- patchwork::wrap_plots(
    plotMatrix(hic_1, use.scores = 'balanced', scale = 'log10', limits = c(-4, -1)),
    plotMatrix(hic_1_despeckled, use.scores = 'despeckled', scale = 'log10', limits = c(-4, -1)),
    plotMatrix(hic_1_despeckled5, use.scores = 'despeckled', scale = 'log10', limits = c(-4, -1))
)
```
...
# - Get compartments
hic <- getCompartments(hic, genome = genome, chromosomes = c('XV', 'XVI'))
hic

# - Export compartments as bigwig and bed files
rtracklayer::export(IRanges::coverage(metadata(hic)$eigens, weight = 'eigen'), 'compartments.bw')
rtracklayer::export(
