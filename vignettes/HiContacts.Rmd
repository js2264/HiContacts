...
## Importing `.(m)/cool` files as `contacts` objects
...
scores(contacts)
scores(contacts, 'random')
```

### Features 

Add any `features` using a `GRanges`. 

```{r}
features(contacts, 'viewpoints') <- GRanges("II:300000-320000")
features(contacts)
features(contacts, 'viewpoints')
```
...
contacts_subset <- contacts[
    start(anchors(contacts)[[1]]) >= 25000 & end(anchors(contacts)[[1]]) <= 30000 & 
    start(anchors(contacts)[[2]]) >= 25000 & end(anchors(contacts)[[2]]) <= 30000
]
